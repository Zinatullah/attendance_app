...
const SpecificUser = () => {
  ...
  useEffect(() => {
    const handleSubmit = async () => {
      const data = await dispatch(getUserAttendance(userId));
                  ...
                  {user ? (
                    user.map((element, index) => (
                      <>
                        <tr className="hidden">
                          <td>
                            {(pickedTime = element.date.split(" ")[4]).toString()}
                            {(str1 = pickedTime.split(":"))}
                            {totalSeconds1 = parseInt( str1[0] * 3600 + str1[1] * 60 + str1[2] )}
                            {parseInt(totalSeconds1) > parseInt(validTime) ? colorPicker= "bg-red-400" : colorPicker="bg-sky-400"}


                          </td>

                            {/* {element.date.split(" ")[4].split(":")[0] > 8
                              ? (colorPicker = "bg-red-400")
                              : (colorPicker = "bg-sky-400")} */}
                          {/* {element.date.split(" ")[4].split(":")[1] > 15
                            ? console.log(
                                "Minutes",
                                element.date.split(" ")[4].split(":")[1]
                              )
                            : console.log(
                                "Minutes",
                                element.date.split(" ")[4].split(":")[1]
                    ...
                    <tr
                      key={999999}
                      className="bg-gray-100  border-b text-sm text-black-600"
                    >
                      <td>No Attendance</td>
                    </tr>
                  )}
